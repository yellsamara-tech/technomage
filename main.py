...
from db import (
    ...
    get_admins, make_admin, revoke_admin, delete_user, get_status_history
)
...
CREATOR_ID = int(os.getenv("CREATOR_ID", "0"))  # ID создателя
...
admin_kb = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton("📈 Статистика по статусам")],
        [KeyboardButton("✉️ Сделать рассылку")],
        [KeyboardButton("📜 История пользователя")]
    ],
    resize_keyboard=True
)
...
async def admin_delete_user(message: types.Message):
    ...
    kb = InlineKeyboardMarkup(row_width=1)
    for u in users:
        if u["id"] != CREATOR_ID and not u["is_admin"]:
...
async def admin_status_stats(message: types.Message):
    ...
    for u in users:
        ...
        for h in history:
            if h["status_date"] == today:
                stats[h["status"]] = stats.get(h["status"], 0) + 1
    text = f"📊 Статистика на {today}:\n"
    for k, v in stats.items():
        text += f"{k}: {v}\n"
    await message.answer(text or "Нет данных за сегодня.")
...
async def admin_history(message: types.Message):
    ...
    for u in users:
        history = await get_status_history(u["id"])
        hist_text = ", ".join([f"{h['status_date']}: {h['status']}" for h in history[-5:]])
